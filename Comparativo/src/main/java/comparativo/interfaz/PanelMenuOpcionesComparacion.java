...
public class PanelMenuOpcionesComparacion extends JPanel implements ActionListener{
    ...
    private JButton btnModificarTodosDescuento;
    ...
    public PanelMenuOpcionesComparacion(InterfazComparativo pInterfaz){
        ...
        btnModificarTodosDescuento = new JButton("Modificar Todos Descuentos");
        btnModificarTodosDescuento.setActionCommand("MODIFICARTODO");
        btnModificarTodosDescuento.addActionListener(this);
        add(btnModificarTodosDescuento);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if("MODIFICARTODO".equals(e.getActionCommand())){
            dialog = new JDialog();
            dialog.setSize(450,175);
            NumberFormat numberFormat = NumberFormat.getNumberInstance(Locale.getDefault());
            DecimalFormat decimalFormat = (DecimalFormat) numberFormat;
            ...
            ActionListener actionListener = new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    double descuentoCompetencia2 = field4.getValue() == null ?-1:((Long) field4.getValue()).doubleValue();
                    if(descuentoPropio<=100 && descuentoCompetencia <=100 && descuentoPropio2<=100 && descuentoCompetencia2<=100){
                        interfaz.actualizarTodosDescuentos(descuentoPropio,descuentoCompetencia, descuentoPropio2, descuentoCompetencia2);
                    }
                    dialog.dispose();
        ...
        if("CALCULARC".equals(e.getActionCommand())){
            dialog = new JDialog();
            dialog.setSize(450,175);
            dialog.setTitle("Actualizando Comparaciones");
            JPanel p3 = new JPanel(new GridLayout(1,1));
            p3.add(new JLabel("Actualizando Comparaciones"));
            dialog.add(p3);
            dialog.setLayout(new BorderLayout());
            dialog.setLocationRelativeTo(null);
            dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
    ...
    @SuppressWarnings("rawtypes")
    private static void backgroundLoad2(){
        ...
        SwingWorker sw1 = new SwingWorker() {
            ...
            protected String doInBackground() throws Exception {
                try {
                    ...
                    String termino = "Termino la actualización de comparaciones";
                    return termino;
                }catch(Exception ex){
            ...
            protected void done(){
                try {
                    ...
                    e.printStackTrace();
                    JOptionPane.showMessageDialog(new JFrame(), "Error, contactar al administrador", "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        };
        sw1.execute();
